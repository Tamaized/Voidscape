...
public class NullWhiteBlockModelHolder extends BlockModelHolder {
	...
	protected DeferredHolder<Block, ? extends Block> blockForName() {
		return blocks.nullBiomeBlocks().NULL_WHITE;
	}
	...
	public Identifier finalize(BlockModelGenerators provider, FurtherExtendedModelTemplateBuilder model) {
		return model
			.buildExtended(m -> m.ambientOcclusion(false))
	...
	protected void defineTextureSlots(ExtendedTextureMapping mapping) {
		mapping
			...
			.putForced(TextureSlot.ALL, new Material(Identifier.fromNamespaceAndPath(Voidscape.MODID, "block/white")));
	}
