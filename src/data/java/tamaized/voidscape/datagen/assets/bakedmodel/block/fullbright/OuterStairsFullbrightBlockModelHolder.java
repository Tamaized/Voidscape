...
public class OuterStairsFullbrightBlockModelHolder extends BlockModelHolder {
	...
	public ModelFile build(BlockModelProvider provider) {
		// @formatter:off
		return provider.getBuilder("block/fullbright/outer_stairs")
			.ao(false)
			...
			.element()
				...
				.shade(false)
				.face(Direction.UP).uvs(8, 8, 16, 16).texture("#top").emissivity(15, 15).cullface(Direction.UP).end()
				.face(Direction.NORTH).uvs(0, 0, 8, 8).texture("#side").emissivity(15, 15).end()
				.face(Direction.SOUTH).uvs(8, 0, 16, 8).texture("#side").emissivity(15, 15).cullface(Direction.SOUTH).end()
				.face(Direction.WEST).uvs(8, 0, 16, 8).texture("#side").emissivity(15, 15).end()
				.face(Direction.EAST).uvs(0, 0, 8, 8).texture("#side").emissivity(15, 15).cullface(Direction.EAST).end()
			.end();
		// @formatter:on
	}
