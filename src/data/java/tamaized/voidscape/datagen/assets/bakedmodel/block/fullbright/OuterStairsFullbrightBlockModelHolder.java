...
@Component
public class OuterStairsFullbrightBlockModelHolder extends BlockModelHolder {
		...
		return model
			.buildExtended(m -> m
				...
				.element(e -> e
					...
					.shade(false)
					.face(Direction.UP, f -> f.uvs(8, 8, 16, 16).texture(TextureSlot.TOP).lightEmission(15).cullface(Direction.UP))
					.face(Direction.NORTH, f -> f.uvs(0, 0, 8, 8).texture(TextureSlot.SIDE).lightEmission(15))
					.face(Direction.SOUTH, f -> f.uvs(8, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.SOUTH))
					.face(Direction.WEST, f -> f.uvs(8, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15))
					.face(Direction.EAST, f -> f.uvs(0, 0, 8, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.EAST))))
			.create(Identifier.fromNamespaceAndPath(Voidscape.MODID, "block/fullbright/outer_stairs"), textures(), provider.modelOutput);
	}
