		...
		return model
			.buildExtended(m -> m
				...
				.element(e -> e
					...
					.shade(false)
					.face(Direction.DOWN, f -> f.uvs(0, 0, 16, 16).texture(TextureSlot.BOTTOM).lightEmission(15))
					.face(Direction.UP, f -> f.uvs(0, 0, 16, 16).texture(TextureSlot.TOP).lightEmission(15).cullface(Direction.UP))
					.face(Direction.NORTH, f -> f.uvs(0, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.NORTH))
					.face(Direction.SOUTH, f -> f.uvs(0, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.SOUTH))
					.face(Direction.WEST, f -> f.uvs(0, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.WEST))
					.face(Direction.EAST, f -> f.uvs(0, 0, 16, 8).texture(TextureSlot.SIDE).lightEmission(15).cullface(Direction.EAST))))
			.create(Identifier.fromNamespaceAndPath(Voidscape.MODID, "block/fullbright/slab_top"), textures(), provider.modelOutput);
	}
