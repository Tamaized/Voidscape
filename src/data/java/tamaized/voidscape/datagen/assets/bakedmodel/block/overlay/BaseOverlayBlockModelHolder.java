	...
	public Identifier finalize(BlockModelGenerators provider, FurtherExtendedModelTemplateBuilder model) {
		return model
			.buildExtended(m -> m
				...
				.element(e -> e
					.from(0, 0, 0).to(16, 16, 16)
					.face(Direction.DOWN, f -> f.texture(ModTextureSlots.OVERLAY_DOWN).cullface(Direction.DOWN))
					.face(Direction.UP, f -> f.texture(ModTextureSlots.OVERLAY_UP).cullface(Direction.UP))
					.face(Direction.NORTH, f -> f.texture(ModTextureSlots.OVERLAY_NORTH).cullface(Direction.NORTH))
					.face(Direction.SOUTH, f -> f.texture(ModTextureSlots.OVERLAY_SOUTH).cullface(Direction.SOUTH))
					.face(Direction.WEST, f -> f.texture(ModTextureSlots.OVERLAY_WEST).cullface(Direction.WEST))
					.face(Direction.EAST, f -> f.texture(ModTextureSlots.OVERLAY_EAST).cullface(Direction.EAST))))
			.create(Identifier.fromNamespaceAndPath(Voidscape.MODID, "block/overlay/base"), textures(), provider.modelOutput);
	}
