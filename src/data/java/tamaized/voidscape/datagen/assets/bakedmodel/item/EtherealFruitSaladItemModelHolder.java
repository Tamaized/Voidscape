...
@Component
public class EtherealFruitSaladItemModelHolder extends ItemModelHolder {
	...
	protected @Nullable DeferredHolder<Item, ? extends Item> itemForName() {
		return items.etherealFruitItems().ETHEREAL_FRUIT_SALAD;
	}
	...
	public ModelFile build(ItemModelProvider provider) {
		return provider.withExistingParent(name(), "item/generated")
			...
			.texture("layer0", name())
			.texture("layer1", name().concat("_overlay"));
	}
