...
@Component
public class AstralChestplateDraconicRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		ItemStack result = new ItemStack(items.modArmorSetComponentDirectory().astralArmorSet().ASTRAL_CHEST.get());
		...
		ExtendedSmithingTransformRecipeBuilder.extendedSmithing(
			...
			Ingredient.of(items.modArmorSetComponentDirectory().astralArmorSet().ASTRAL_CHEST.get()),
			Ingredient.of(Items.DRAGON_EGG),
			RecipeCategory.COMBAT,
			result
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "astral_chestplate_draconic");
	}
