...
@Component
public class AstralPickaxeRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		SmithingTransformRecipeBuilder.smithing(
			...
			Ingredient.of(items.toolSetComponentDirectory().ichorToolSet().ICHOR_PICKAXE.get()),
			Ingredient.of(items.materialItems().ASTRAL_CRYSTAL.get()),
			RecipeCategory.COMBAT,
			items.toolSetComponentDirectory().astralToolSet().ASTRAL_PICKAXE.get()
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "astral_pickaxe");
	}
