...
@Component
public class IchorPickaxeRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		SmithingTransformRecipeBuilder.smithing(
			...
			Ingredient.of(items.toolSetComponentDirectory().titaniteToolSet().TITANITE_PICKAXE.get()),
			Ingredient.of(items.materialItems().ICHOR_CRYSTAL.get()),
			RecipeCategory.COMBAT,
			items.toolSetComponentDirectory().ichorToolSet().ICHOR_PICKAXE.get()
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "ichor_pickaxe");
	}
