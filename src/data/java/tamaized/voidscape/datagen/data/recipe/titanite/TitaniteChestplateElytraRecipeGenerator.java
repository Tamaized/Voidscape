...
@Component
public class TitaniteChestplateElytraRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		ItemStack result = new ItemStack(items.modArmorSetComponentDirectory().titaniteArmorSet().TITANITE_CHEST.get());
		...
		ExtendedSmithingTransformRecipeBuilder.extendedSmithing(
			...
			Ingredient.of(items.modArmorSetComponentDirectory().titaniteArmorSet().TITANITE_CHEST.get()),
			Ingredient.of(Items.ELYTRA),
			RecipeCategory.COMBAT,
			result
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "titanite_chestplate_elytra");
	}
