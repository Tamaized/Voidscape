...
@Component
public class TitanitePickaxeRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		SmithingTransformRecipeBuilder.smithing(
			...
			Ingredient.of(items.toolSetComponentDirectory().voidicCrystalToolSet().VOIDIC_CRYSTAL_PICKAXE.get()),
			Ingredient.of(items.materialItems().TITANITE_SHARD.get()),
			RecipeCategory.COMBAT,
			items.toolSetComponentDirectory().titaniteToolSet().TITANITE_PICKAXE.get()
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "titanite_pickaxe");
	}
