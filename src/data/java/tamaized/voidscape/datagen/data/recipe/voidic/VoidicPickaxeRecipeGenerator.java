...
@Component
public class VoidicPickaxeRecipeGenerator implements IRecipeGenerator {
	...
	public void generate(RecipeOutput recipeOutput) {
		SmithingTransformRecipeBuilder.smithing(
			...
			Ingredient.of(items.materialItems().VOIDIC_CRYSTAL.get()),
			RecipeCategory.COMBAT,
			items.toolSetComponentDirectory().voidicCrystalToolSet().VOIDIC_CRYSTAL_PICKAXE.get()
		)
			.unlocks("has_template", recipeProviderUtil.has(items.augmentItems().VOIDIC_TEMPLATE.get()))
			.save(recipeOutput, "voidic_crystal_pickaxe");
	}
