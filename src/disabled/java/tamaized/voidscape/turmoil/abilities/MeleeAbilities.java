...
public class MeleeAbilities {
	...
	public static final TurmoilAbility RUSH = new TurmoilAbility(unloc("rush"), TurmoilAbility.Type.Voidic, 200, 15 * 20, (spell, caster) -> {
		final boolean flag = caster.level instanceof ServerLevel && ((ServerLevel) caster.level).getChunkSource().getGenerator() instanceof InstanceChunkGenerator;
		HitResult ray = Voidscape.getHitResultFromEyes(caster, e -> !flag || !(e instanceof Player), 16);
		if (caster.level instanceof ServerLevel && ray.getType() != HitResult.Type.MISS) {
			caster.getCapability(SubCapability.CAPABILITY).ifPresent(cap -> cap.get(Voidscape.subCapTurmoilStats).ifPresent(stats -> stats.
					ramTowards(ray instanceof EntityHitResult ? ((EntityHitResult) ray).getEntity().position() : ray.getLocation(), spell.damage())));
			for (int i = 0; i < 25; i++) {
				Vec3 pos = new Vec3(0.25F + caster.getRandom().nextFloat() * 0.75F, 0, 0).
				...
				((ServerLevel) caster.level).sendParticles(ParticleTypes.FIREWORK, pos.x, pos.y, pos.z, 0, 0, 0, 0, 1);
			}
			caster.level.playSound(null, caster, SoundEvents.FIREWORK_ROCKET_LAUNCH, SoundSource.PLAYERS, 1F, 0.75F + caster.getRandom().nextFloat() * 0.5F);
			return true;
		}
		return false;
	}).damage(4);
	...
	public static final TurmoilAbility CLEAVE = new TurmoilAbility(unloc("cleave"), TurmoilAbility.Type.Voidic, 350, 30 * 20, (spell, caster) -> {
		if (caster.getMainHandItem().isEmpty() || !caster.getMainHandItem().getAttributeModifiers(EquipmentSlot.MAINHAND).containsKey(ModAttributes.VOIDIC_DMG.get()))
			return false;
		...
		if (f > 0.0F) {
			...
			if (caster.level instanceof ServerLevel) {
				((ServerLevel) caster.level).sendParticles(ParticleTypes.SWEEP_ATTACK, caster.getX() + d0, caster.getY(0.5D), caster.getZ() + d1, 0, d0, 0.0D, d1, 0.0D);
			}
			for (LivingEntity livingentity : caster.level.getEntitiesOfClass(LivingEntity.class, caster.getBoundingBox().inflate(1F).move(caster.getLookAngle().scale(2F)))) {
				if (livingentity != caster && !caster.isAlliedTo(livingentity) && (!(livingentity instanceof ArmorStand) || !((ArmorStand) livingentity).isMarker()) && caster.distanceToSqr(livingentity) < 9.0D) {
					livingentity.knockback(0.4F, Mth.sin(caster.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(caster.getYRot() * ((float) Math.PI / 180F)));
