...
public class MageSkills implements ITurmoilSkills {
	...
	public final TurmoilSkill CORE = builder("core").noDescription().core(TurmoilSkill.CoreType.Ranged).stats(stats -> stats.threat(1)).abilities(MageAbilities.BOLT).texture(MageAbilities.BOLT.getTexture()).build();

	public final TurmoilSkill VOIDIC_BOND = generic("voidic_bond").required(CORE).stats(stats -> stats.spellpower(5)).texture(GenericSkillIcons.VOIDIC_BOND).build();
	public final TurmoilSkill SPELLPOWER_1 = generic("spellpower_1").description("spellpower").required(VOIDIC_BOND).stats(stats -> stats.spellpower(5)).texture(GenericSkillIcons.SPELLPOWER).build();
	public final TurmoilSkill SPELLPOWER_2 = generic("spellpower_2").description("spellpower").required(SPELLPOWER_1).stats(stats -> stats.spellpower(5)).texture(GenericSkillIcons.SPELLPOWER).build();
	public final TurmoilSkill SPELLPOWER_3 = generic("spellpower_3").description("spellpower").required(SPELLPOWER_2).stats(stats -> stats.spellpower(5)).texture(GenericSkillIcons.SPELLPOWER).build();
	public final TurmoilSkill SPELLPOWER_4 = generic("spellpower_4").description("spellpower").required(SPELLPOWER_3).stats(stats -> stats.spellpower(10)).texture(GenericSkillIcons.SPELLPOWER).build();
	public final TurmoilSkill SPELLPOWER_5 = generic("spellpower_5").description("spellpower").required(SPELLPOWER_4).stats(stats -> stats.spellpower(20)).texture(GenericSkillIcons.SPELLPOWER).build();
	public final TurmoilSkill BONDING_1 = generic("voidic_bonding_1").description("voidic_bonding").required(VOIDIC_BOND).stats(stats -> stats.rechargeRate(1)).texture(GenericSkillIcons.VOIDIC_BONDING).build();
	public final TurmoilSkill BONDING_2 = generic("voidic_bonding_2").description("voidic_bonding").required(BONDING_1).stats(stats -> stats.rechargeRate(1)).texture(GenericSkillIcons.VOIDIC_BONDING).build();
	public final TurmoilSkill BONDING_3 = generic("voidic_bonding_3").description("voidic_bonding").required(BONDING_2).stats(stats -> stats.rechargeRate(1)).texture(GenericSkillIcons.VOIDIC_BONDING).build();
	public final TurmoilSkill BONDING_4 = generic("voidic_bonding_4").description("voidic_bonding").required(BONDING_3).stats(stats -> stats.rechargeRate(2)).texture(GenericSkillIcons.VOIDIC_BONDING).build();
	public final TurmoilSkill BONDING_5 = generic("voidic_bonding_5").description("voidic_bonding").required(BONDING_4).stats(stats -> stats.rechargeRate(5)).texture(GenericSkillIcons.VOIDIC_BONDING).build();
	public final TurmoilSkill SPELL_DEX_1 = generic("spell_dex_1").description("spell_dex").required(VOIDIC_BOND).stats(stats -> stats.cooldown(5)).texture(GenericSkillIcons.SPELL_DEX).build();
	public final TurmoilSkill SPELL_DEX_2 = generic("spell_dex_2").description("spell_dex").required(SPELL_DEX_1).stats(stats -> stats.cooldown(5)).texture(GenericSkillIcons.SPELL_DEX).build();
	public final TurmoilSkill SPELL_DEX_3 = generic("spell_dex_3").description("spell_dex").required(SPELL_DEX_2).stats(stats -> stats.cooldown(5)).texture(GenericSkillIcons.SPELL_DEX).build();
	public final TurmoilSkill SPELL_DEX_4 = generic("spell_dex_4").description("spell_dex").required(SPELL_DEX_3).stats(stats -> stats.cooldown(5)).texture(GenericSkillIcons.SPELL_DEX).build();
	public final TurmoilSkill SPELL_DEX_5 = generic("spell_dex_5").description("spell_dex").required(SPELL_DEX_4).stats(stats -> stats.cooldown(10)).texture(GenericSkillIcons.SPELL_DEX).build();
	public final TurmoilSkill ASTUTE_UNDERSTANDING_1 = generic("astute_understanding_1").description("astute_understanding").required(VOIDIC_BOND).stats(stats -> stats.cost(1)).texture(GenericSkillIcons.ASTUTE_UNDERSTANDING).build();
	public final TurmoilSkill ASTUTE_UNDERSTANDING_2 = generic("astute_understanding_2").description("astute_understanding").required(ASTUTE_UNDERSTANDING_1).stats(stats -> stats.cost(1)).texture(GenericSkillIcons.ASTUTE_UNDERSTANDING).build();
	public final TurmoilSkill ASTUTE_UNDERSTANDING_3 = generic("astute_understanding_3").description("astute_understanding").required(ASTUTE_UNDERSTANDING_2).stats(stats -> stats.cost(1)).texture(GenericSkillIcons.ASTUTE_UNDERSTANDING).build();
	...
	public final TurmoilSkill VOIDMANCY_1 = builder("voidmancy_1").noDescription().required(CORE).stats(stats -> stats.spellpower(2).rechargeRate(1).spellCrit(1)).texture("skills/mage/voidmancy").build();
	public final TurmoilSkill HOMINGBOLTS = builder("homingbolts").noDescription().required(VOIDMANCY_1).stats(stats -> stats.spellpower(5)).abilities(MageAbilities.HOMING_BOLTS).texture(MageAbilities.HOMING_BOLTS.getTexture()).build();
	public final TurmoilSkill VOIDICAURA = builder("voidicaura").noDescription().required(VOIDMANCY_1).abilities(MageAbilities.AURA).texture(MageAbilities.AURA.getTexture()).build();
	public final TurmoilSkill VOIDMANCY_2 = builder("voidmancy_2").required(VOIDMANCY_1).stats(stats -> stats.spellpower(2).cooldown(1).spellCrit(1)).texture(VOIDMANCY_1.getTexture()).build();
	...
	public final TurmoilSkill VOIDMANCY_3 = builder("voidmancy_3").required(VOIDMANCY_2).stats(stats -> stats.spellpower(2).rechargeRate(1).spellCrit(1)).texture(VOIDMANCY_1.getTexture()).build();
	...
	public final TurmoilSkill VOIDMANCY_4 = builder("voidmancy_4").required(VOIDMANCY_3).stats(stats -> stats.spellpower(2).cooldown(1).spellCrit(1)).texture(VOIDMANCY_1.getTexture()).build();
	...
	public final TurmoilSkill VOIDMANCY_5 = builder("voidmancy_5").required(VOIDMANCY_4).stats(stats -> stats.spellpower(12).rechargeRate(3).cooldown(3).spellCrit(6)).texture(VOIDMANCY_1.getTexture()).build();
