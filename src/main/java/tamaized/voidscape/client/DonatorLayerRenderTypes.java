...
public class DonatorLayerRenderTypes {
	...
	public final Supplier<RenderType> WINGS = Suppliers.memoize(() -> RenderType.create(
		"voidscape_wings",
		wingsSetup().createRenderSetup()
	));

	public final Supplier<RenderType> WINGS_ITEM_TARGET = Suppliers.memoize(() -> RenderType.create(
		"voidscape_wings_item_target",
		wingsSetup().setOutputTarget(OutputTarget.ITEM_ENTITY_TARGET).createRenderSetup()
	));

	private RenderSetup.RenderSetupBuilder wingsSetup() {
		return renderSetup(shaders.VOIDSKY_WINGS).withTexture("Sampler1", AbstractEndPortalRenderer.END_PORTAL_LOCATION);
	}
