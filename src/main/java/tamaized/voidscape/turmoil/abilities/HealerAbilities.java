...
public class HealerAbilities {
	...
	public static final TurmoilAbility REZ = new TurmoilAbility(unloc("resurrection"), TurmoilAbility.Type.Voidic, 800, 30 * 20, (spell, caster) -> {
		HitResult ray = Voidscape.getHitResultFromEyes(caster, e -> e.getCapability(SubCapability.CAPABILITY).map(cap -> cap.get(Voidscape.subCapTurmoilTracked).map(data -> data.incapacitated).orElse(false)).orElse(false), 32, 1.5D, 0D);
		if (caster.level instanceof ServerLevel && ray instanceof EntityHitResult) {
			Entity entity = ((EntityHitResult) ray).getEntity();
			...
			for (int i = 0; i < 10; i++) {
				Vec3 pos = new Vec3(0.25F + entity.level.random.nextFloat() * 0.75F, 0, 0).yRot((float) Math.toRadians(entity.level.random.nextInt(360))).add(entity.getX(), entity.getY() + entity.level.random.nextFloat(), entity.getZ());
				((ServerLevel) caster.level).sendParticles(ParticleTypes.END_ROD, pos.x, pos.y, pos.z, 0, 0, 0, 0, 1);
			}
			caster.level.playSound(null, entity, SoundEvents.BEACON_POWER_SELECT, SoundSource.PLAYERS, 1F, 0.75F + entity.level.random.nextFloat() * 0.5F);
			return true;
		}
		return false;
	});
	...
	public static final TurmoilAbility MEND = new TurmoilAbility(unloc("mend"), TurmoilAbility.Type.Voidic, 100, 3 * 20, (spell, caster) -> {
		HitResult ray = Voidscape.getHitResultFromEyes(caster, e -> e instanceof LivingEntity, 2);
		if (caster.level instanceof ServerLevel && ray instanceof EntityHitResult) {
