...
public class HealerSkills implements ITurmoilSkills {
	...
	public final TurmoilSkill CORE = builder("core").noDescription().core(TurmoilSkill.CoreType.Healer).stats(stats -> stats.threat(1)).abilities(HealerAbilities.REZ).texture(HealerAbilities.REZ.getTexture()).build();

	public final TurmoilSkill VOIDIC_BOND = generic("voidic_bond").required(CORE).stats(stats -> stats.spellpower(5).rechargeRate(1)).texture(GenericSkillIcons.VOIDIC_BOND).build();
