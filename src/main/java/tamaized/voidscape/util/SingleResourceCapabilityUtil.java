...
public class SingleResourceCapabilityUtil {
	...
	public <S, T extends Resource> int extract(StacksResourceHandler<S, T> handler, T resource, int amount, TransactionContext transaction) {
	...
	public <S, T extends Resource> int capacity(StacksResourceHandler<S, T> handler, T resource) {
		return handler.getCapacityAsInt(0, resource);
	}
	...
	public <S, T extends Resource> int extract(Supplier<? extends StacksResourceHandler<S, T>> handler, T resource, int amount, TransactionContext transaction) {
		return handler.get().extract(resource, amount, transaction);
	}
	...
	public <S, T extends Resource> int capacity(Supplier<? extends StacksResourceHandler<S, T>> handler, T resource) {
		return handler.get().getCapacityAsInt(0, resource);
	}
